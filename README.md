...
### <a href="#oss">오픈소스 활용 가치</a>
### <a href="#security">책임 있는 사용</a>
...
기존 IDS/IPS 학습 예제는 오프라인 데이터셋 분류에 머무르는 경우가 많습니다. DeepFence는 학습된 모델이 실제 네트워크 이벤트와 만났을 때 어떤 문제가 생기는지 확인하고, 탐지 결과를 정책 판단과 차단 경로까지 연결하는 실험 환경을 제공하기 위해 만들었습니다.

이 프로젝트의 목표는 다음과 같습니다.

- 보안 연구자와 개발자가 재현 가능한 AI 기반 IDS/IPS 파이프라인을 실험할 수 있게 만들기
- CIC-IDS 계열 데이터셋 학습 결과를 Suricata 런타임 이벤트와 연결하기
- ML confidence만으로 차단하지 않고, 시그니처/행위/반복 관측/화이트리스트를 함께 고려하는 정책 엔진 제공
- 탐지 이벤트를 OpenSearch에 저장해 운영 관점의 분석과 시각화를 가능하게 하기
- 향후 커뮤니티가 룰, 피처, 모델, 정책을 독립적으로 개선할 수 있는 구조 만들기

현재 DeepFence는 프로덕션 완성형 보안 제품이 아니라, 오픈소스 기반으로 개선 가능한 실험형 보안 파이프라인입니다.
...
OpenSearch, 모델 경로, 차단 정책 등 런타임 설정은 `.env` 파일로 관리합니다.

```bash
cp service/configs/.env.example service/configs/.env
```

필요에 따라 `service/configs/.env` 값을 수정합니다.

## 빠른 실행

<a name="quick-start"></a>

### 1. 샘플 플로우 기반 detect-only 실행

Suricata나 root 권한 없이 로컬 샘플 플로우로 탐지 파이프라인을 확인합니다.
...
실시간 네트워크 인터페이스를 감시하려면 Suricata 설치와 root 권한이 필요합니다.
...
- [NetFlow v1 학습 파이프라인](./docs/NETFLOW_V1.md)
...
DeepFence는 다음과 같은 사용자와 기여자를 위해 공개됩니다.

- AI 기반 네트워크 보안 탐지를 학습하거나 연구하는 개발자
- Suricata 이벤트를 ML 모델 입력으로 변환하는 구조를 실험하려는 보안 연구자
- IDS/IPS 정책 엔진, 차단 정책, 탐지 이벤트 저장 구조를 참고하려는 오픈소스 메인테이너
- CIC-IDS-2018, NF-CSE-CIC-IDS2018 등 공개 데이터셋 기반 모델을 실제 런타임과 연결해보고 싶은 사용자

기여 가능한 영역은 다음과 같습니다.

- 실시간 Suricata 이벤트와 학습 피처 간 매핑 개선
- 공격 유형별 모델 성능 평가 및 재학습
- HTTP, DNS, TLS, Flow 기반 시그니처 추가
- 오탐을 줄이기 위한 정책 엔진 개선
- OpenSearch 대시보드, 문서, 테스트 케이스 보강

## 책임 있는 사용

<a name="security"></a>

DeepFence는 방어 목적의 네트워크 보안 연구와 학습을 위해 만들어졌습니다. 본 프로젝트의 코드는 본인이 소유하거나 명시적으로 허가받은 네트워크 환경에서만 사용해야 합니다.

주의 사항:

- 무단 네트워크 감시, 공격, 침투 시도에 사용하지 마세요.
- 자동 차단 기능은 정상 트래픽을 차단할 수 있으므로 테스트 환경에서 먼저 검증하세요.
- 현재 모델과 정책은 실험 단계이며, 운영 환경의 유일한 보안 통제 수단으로 사용해서는 안 됩니다.
- 데이터셋 기반 학습 결과와 실제 네트워크 트래픽 분포는 다를 수 있습니다.
